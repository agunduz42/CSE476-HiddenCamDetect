...
def extract_features(input_jsonl: Path, output_csv: Path, no_streaming_label=1):
    # group packets by explicit flow_id if present, otherwise by generated id
    ...
    with input_jsonl.open() as f:
        for line in f:
            ...
            if not flow_id:
                # fallback grouping: src_mac + dst_ip + dst_port + protocol + minute window
                ts = r.get("ts")
    ...
    with out_path.open("w", newline="") as csvf:
        ...
        for (src_mac, fid), pkts in flows.items():
            ...
            # inter-arrival times for initiator (upload) packets only
            iat = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", "-i", default="data/mock_pcap/mock_packets.jsonl")
    parser.add_argument("--output", "-o", default="data/processed/flows_features_no_streaming.csv")
    parser.add_argument("--label", "-l", type=int, default=1, help="no_streaming label (1=no-streaming, 0=streaming)")
